...
import io.github.railroad.discord.DiscordPlugin;
...
public final class DiscordCore implements AutoCloseable {
    ...
    private void handleCommand(DiscordCommand command) {
        if (command.isError()) {
            this.logger.error("Received error from Discord IPC channel: {}", command);
            return;
        }
    ...
    public void onReady() {
        ...
        registerEvents();
        this.logger.info("Discord IPC channel is ready");
    ...
    private void registerEvents() {
        for (Map.Entry<DiscordCommand.Event, DiscordEventHandler<?>> handler : events.getHandlers()) {
            ...
            sendCommand(command, response -> this.logger.debug("Registered event {}", event.name()));
        }
    }
    ...
    public DiscordResult checkError(DiscordCommand command) {
        if (command.getEvt() == DiscordCommand.Event.ERROR) {
            ...
            this.logger.error("Received error from Discord IPC channel: {}", error.getMessage());
